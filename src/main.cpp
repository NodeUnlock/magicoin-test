...
#if defined(NDEBUG)
# error "Magicoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xd861236d710682e2492629e648aaffe8606b91474d58d365587f0598cbc22591");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Magicoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Magicoin Signed Message:\n";
...
{
    // Magicoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 200000); // Magicoin: 20k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Magicoin: 1 day
static const int64 nTargetSpacing = 1.5 * 60; // Magicoin: 1.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Magicoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Magicoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x790df032b17dd48e291118549ac5419f3cfa5c75ce0c863a0e67726fb0299591");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "LONDON - 5th April 18 - First nodeunlock crypto and Verge XVG gets hacked.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04627ca9fa2b8e224bea67cebc429e17f3cf9db030b3237612c58e02db72ec4611b3700dffa7a7d908e794fe927526619e56978fde10a6dae0ced19ce9f4c3d9e0") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1522950595;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084879506;

        if (fTestNet)
        {
            block.nTime    = 1522950567;
            block.nNonce   = 386657972;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xfd3dfd5ce874dbdd408874ca4de1b651d7740b3be4652d230b512de2a8084fbe"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf2, 0xc9, 0xbd, 0xd7 }; // Magicoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MagicoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MagicoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static MagicoinMiner(CWallet *pwallet)
{
    printf("MagicoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("magicoin-miner");
    ...
    try { loop {
        ...
        printf("Running MagicoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("MagicoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MagicoinMiner, pwallet));
}
